...
class MainWindow(CTk):
    def __init__(self):
        ...
        try:#!!!!!!!!!!! 
            self.sock.connect(("7.tcp.eu.ngrok.io", 19348)) #!!!!БУДЕМО МІНЯТИ ПОТІМ
        ...
        self.label_theme = CTkOptionMenu(self.frame, values = ["Dark", "Light", "Myyy"], 
        ...
        self.username = self.entry.get() or "Vityok" #Отримати запис в CTkEntry
        try:
    ...
    def change_theme (self, value):
        if value == "Dark":
            ...
            self.configure(bg_color="#2a1a40", fg_color="#2a1a40")
            self.frame.configure(fg_color="#2a1a40")

            self.label.configure(text_color="#e0b3ff")
            self.entry.configure(fg_color="#3d2b5e", text_color="#e0b3ff")
            self.chat_field.configure(fg_color="#2a1a40")

            self.message_input.configure(fg_color="#3d2b5e", text_color="#e0b3ff")
            self.send_button.configure(fg_color="#3d2b5e", text_color="#e0b3ff")
            self.btn.configure(fg_color="#3d2b5e", text_color="#e0b3ff")
            self.label_theme.configure(fg_color="#3d2b5e", text_color="#e0b3ff")

            
            for i in self.chat_field.winfo_children():
                if isinstance(i, CTkLabel):
                    i.configure(text_color="#e0b3ff")
